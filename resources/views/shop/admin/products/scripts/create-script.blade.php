...
<script>
	...
	function fileComplete (e, file, response) {
		...
		if($.parseJSON(response).type == 'gallery') {
			...
			$('#loading-gallery').css('display', 'none');
			console.log($.parseJSON(response).session);
		}
...
<script type="text/javascript">
	$('#submitForm').on('click', function(e) {
		if(!$('.textarea').summernote('isEmpty')){
			if($('#preview_main').find('a').length) {
				if(isEmpty($('#preview_gallery').find('div').length)) {
					e.preventDefault();
					Swal.fire({
						title: 'Вы точно хотите добавить товар без галереи?',
						icon: 'warning',
						showCancelButton: true,
						confirmButtonColor: '#28a745',
						cancelButtonColor: '#d33',
